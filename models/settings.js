...
NEWSCHEMA('Settings').make(function(schema) {
	...
	schema.addWorkflow('load', function(error, model, options, callback) {
		schema.get(null, function(err, settings) {
            ...
            F.config.custom.templates = [];
            
            Fs.readdirSync(__dirname + '/../themes/' + CONFIG('default-theme') + '/views/cms').forEach(function (file) {
                
                if (!file.endsWith('.html')) // exclude not json
                        return;
                
                F.config.custom.templates.push(file.substring(0,file.length-5));
            });

            //console.log(F.config.custom.templates);
